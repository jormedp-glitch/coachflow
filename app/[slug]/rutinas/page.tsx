...
export default function RutinasPage({ params }: { params: Promise<{ slug: string }> }) {
  ...
  async function guardarRutina() {
    ...
    const { error } = await supabase.from('cf_rutinas').insert({
      ...
      semanas_total: formRutina.semanas_total,
      activo: true
    })
    if (!error) {
      setFormRutina({ nombre: '', descripcion: '', semanas_total: 4 })
      setMostrarFormRutina(false)
      cargarDatos()
    }
    setGuardando(false)
  }
  ...
  async function guardarEjercicio() {
    ...
    if (!error) {
      setFormEjercicio({ nombre: '', grupo_muscular: '', url_video: '', descripcion: '' })
        ...
        {vista === 'rutinas' && (
          <>
            <div className="flex items-center justify-between mb-4">
              ...
              <button
                onClick={() => setMostrarFormRutina(!mostrarFormRutina)}
              ...
              <div className="bg-zinc-900 border border-zinc-800 rounded-xl p-5 mb-5 space-y-4">
                ...
                <div className="grid grid-cols-2 gap-4">
                  ...
                  <div>
                    ...
                    <select
                      value={formRutina.semanas_total}
                      onChange={e => setFormRutina({ ...formRutina, semanas_total: Number(e.target.value) })}
                ...
                <div>
                  ...
                  <input
                    ...
                    className="w-full bg-zinc-800 text-white rounded-lg px-3 py-2 text-sm border border-zinc-700 focus:outline-none focus:border-zinc-500"
                    placeholder="Para quien es esta rutina, objetivo general..."
                  />
                </div>
                <div className="flex gap-3">
                  ...
                  <button onClick={() => setMostrarFormRutina(false)} className="text-zinc-500 hover:text-white text-sm px-4 py-2 rounded-lg transition-colors">
                    Cancelar
                  </button>
                </div>
              </div>
            ...
            <div className="space-y-3">
              {rutinas.length === 0 ? (
                ...
                rutinas.map(r => (
                  <div key={r.id} className="bg-zinc-900 border border-zinc-800 rounded-xl p-4 flex items-center justify-between">
                    <div>
                    ...
                    <button
                      onClick={() => router.push('/' + slug + '/rutinas/' + r.id)}
              ...
              <div className="bg-zinc-900 border border-zinc-800 rounded-xl p-5 mb-5 space-y-4">
                ...
                <div className="grid grid-cols-2 gap-4">
                  ...
                  <div>
                    ...
                    <input
                      ...
                      className="w-full bg-zinc-800 text-white rounded-lg px-3 py-2 text-sm border border-zinc-700 focus:outline-none focus:border-zinc-500"
                      placeholder="Piernas, Core, Pecho..."
                    />
                  </div>
                </div>
                <div>
                  ...
                  <input
                    type="text"
                    value={formEjercicio.url_video}
                    onChange={e => setFormEjercicio({ ...formEjercicio, url_video: e.target.value })}
                    className="w-full bg-zinc-800 text-white rounded-lg px-3 py-2 text-sm border border-zinc-700 focus:outline-none focus:border-zinc-500"
                ...
                <div>
                  ...
                  <textarea
                    ...
                    className="w-full bg-zinc-800 text-white rounded-lg px-3 py-2 text-sm border border-zinc-700 focus:outline-none focus:border-zinc-500"
                    rows={2}
                    placeholder="Tecnica, puntos clave..."
                  />
                </div>
                <div className="flex gap-3">
                  ...
                  <button onClick={() => setMostrarFormEjercicio(false)} className="text-zinc-500 hover:text-white text-sm px-4 py-2 rounded-lg transition-colors">
                    Cancelar
                  </button>
                </div>
              </div>
